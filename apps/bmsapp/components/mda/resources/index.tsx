...
import AddFolder from './resource/AddFolder';
import { fetcher } from 'libs/fetcher';
import FolderProperties from './resource/FolderProperties';
import { Resource } from '@prisma/client';
import { useRouter } from 'next/router';
import useSWR from 'swr';
import { StaleHolder } from 'components/all/skeletons';
import AddFile from './resource/AddFile';
...
export const Resources = () => {
  ...
  const {
    ...
    isLoading,
    isValidating,
  } = useSWR<TApiResult>(`/api/resources/${lastResource}`, fetcher);

  const busy = Boolean(isLoading || isValidating);

  // const { data: resourceData = [], isLoading } = useSWR<TApiResult>(
  // get all resources
  ...
  return (
    <>
      <div
        className={`${
          hasParent ? 'grid grid-cols-12 w-full' : 'grid w-full h-screen'
        }`}
      >
        <div
          className={`${
            hasParent ? '2xl:col-span-9 col-span-12 h-screen w-full' : 'w-full'
          } `}
        >
          <h3 className="text-3xl font-bold text-bgray-900 dark:text-white mb-10">
            Files and Folders
          ...
          <div className="flex flex-wrap lg:gap-x-0 gap-3 gap-y-5">
            <StaleHolder
              ...
              height={170}
            >
              {resourceData?.data?.map((item: Resource) => {
